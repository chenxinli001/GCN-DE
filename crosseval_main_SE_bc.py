...
def masked_l1_distance(f1,f2,mask):
    bs = f1.shape[0]
    f1 = f1.view(bs,-1)
    f2 = f2.view(bs,-1)
    mask = mask.view(bs,-1)
    distance = torch.abs((f1-f2)*mask).sum() / (mask==1).sum()
    return distance
...
if DataSet == 'MRI':
    ...
    Num_support = 8
    lambda_t = args.t
    model_path = './result_correct/CT/'


model_path = model_path + ObjectOrgan + '/t{}_bc_SE{}'.format(lambda_t,args.SE_type)
...
if not args.test_vis:
    for k_fold in range(5):
        ...
        net = fss_fea(args.pretrain,args.SE_type).cuda()
        ...
        for e in range(start_epoch,num_epoch+1):
            ...
            for i_batch, sampled_batch in enumerate(train_loader):
                ...
                sf= torch.mean(s_feature[4],dim=1)
                qf = torch.mean(q_feature[4],dim=1)
                qwf = torch.mean(qw_feature[4],dim=1)

                # print(support_label.shape)
                fg_mask = F.interpolate(support_label.unsqueeze(1),size=[16,16])
                # print(fg_mask.shape)
                bg_mask = 1 - fg_mask
                bg_mask = bg_mask.squeeze(1)
                #print(np.unique(bg_mask.cpu().numpy()))

                tri_loss = masked_l1_distance(sf,qf,bg_mask)
