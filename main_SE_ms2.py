...
for e in range(start_epoch,num_epoch+1):
    ...
    for i_batch, sampled_batch in enumerate(train_loader):
        ...
        if len(_query_label)==1:
            support_image, query_image, support_label, query_label = split_batch(image,label,_query_label[0])
            ...
            optimizer.zero_grad()
            for k in range(len(_query_label)):
                support_image, query_image, support_label, query_label = split_batch(image,label,_query_label[k])
        
                condition_input = torch.cat([support_image,support_label.unsqueeze(dim=1)],dim=1)
                seg = net(condition_input,query_image)
        
                loss = criterion1(seg, query_label) + criterion2(seg.squeeze(dim=1), query_label)
                #loss /= len(_query_label)
                loss.backward(retain_graph=True)

                if k == len(_query_label)-1:
                    optimizer.step()
                    optimizer.zero_grad()
