...
model_path = \
    './result_ex/MRI/liver/ex_pretrain_/0-fold_best.pth.tar'
k_fold = 0

# vis_layer = [5]

net = fss_feature_ex1(pretrain=True).cuda()
...
with torch.no_grad():
    ...
    for i_batch, sampled_batch in enumerate(train_loader):
        ...
        attention_map_list = [net.segmentor.w1.attention.cpu().numpy().squeeze(),net.segmentor.w2.attention.cpu().numpy().squeeze(),
            net.segmentor.w3.attention.cpu().numpy().squeeze(),net.segmentor.w4.attention.cpu().numpy().squeeze(),
            net.segmentor.w5.attention.cpu().numpy().squeeze(),net.segmentor.w6.attention.cpu().numpy().squeeze(),
            net.segmentor.w7.attention.cpu().numpy().squeeze(),net.segmentor.w8.attention.cpu().numpy().squeeze(),
            net.segmentor.w9.attention.cpu().numpy().squeeze()]

        for i in range(9):
            att = attention_map_list[i]
            att_map1[i] = cv2.resize(att,(256,256))
        ...
        for i in range(9):
            att = attention_map_list[i]
            att_map2[i] = cv2.resize(att,(256,256))
            fea = q_feature[i][0].cpu().numpy()
            w_fea  = qw_feature[i][0].cpu().numpy()
