...
def tri_loss_c1(fs,fq,distance,train_bs):
    tri_loss1 = distance(fs[0:train_bs],fq[0:train_bs])
    tri_loss1 =  torch.log(1+torch.exp(tri_loss1))
    return tri_loss1

def tri_loss_c2(fs,fq,distance,train_bs):
    tri_loss1 = ( distance(fs[0:train_bs],fq[0:train_bs]) - 
                distance(fs[0:train_bs],fq[train_bs:train_bs*2]) )
    tri_loss2 = ( distance(fs[train_bs:train_bs*2],fq[train_bs:train_bs*2]) -
                distance(fs[train_bs:train_bs*2],fq[0:train_bs]) )
    tri_loss1 =  torch.log(1+torch.exp(tri_loss1))/2
    tri_loss2  = torch.log(1+torch.exp(tri_loss2))/2
    return tri_loss1,tri_loss2

def tri_loss_c3(fs,fq,distance,train_bs):
    tri_loss12 = ( distance(fs[0:train_bs],fq[0:train_bs]) - 
                distance(fs[0:train_bs],fq[train_bs:train_bs*2]) )
    tri_loss13 = ( distance(fs[0:train_bs],fq[0:train_bs]) - 
        distance(fs[0:train_bs],fq[train_bs*2:train_bs*3]) )

    tri_loss21 = ( distance(fs[train_bs:train_bs*2],fq[train_bs:train_bs*2]) - 
        distance(fs[train_bs:train_bs*2],fq[0:train_bs]) )
    tri_loss23 = ( distance(fs[train_bs:train_bs*2],fq[train_bs:train_bs*2])-
        distance(fs[train_bs:train_bs*2],fq[train_bs*2:train_bs*3]) )
    
    tri_loss31 = ( distance(fs[train_bs*2:train_bs*3],fq[train_bs*2:train_bs*3]) - 
        distance(fs[train_bs*2:train_bs*3],fq[0:train_bs]) )
    tri_loss32 = ( distance(fs[train_bs*2:train_bs*3],fq[train_bs*2:train_bs*3]) -
        distance(fs[train_bs*2:train_bs*3],fq[train_bs:train_bs*2]) )
    ...
    return tri_loss1, tri_loss2, tri_loss3
...
for e in range(start_epoch,num_epoch+1):
    ...
    for i_batch, sampled_batch in enumerate(train_loader):
        ...
        # for i in range(len(q_feature)):
        #     print(q_feature[i].shape,qw_feature[i].shape)
        ...
        #q_feature = qw_feature

        if len(_query_label)==1:
            if SC_DIS==0:
                loss = seg_loss
            else:
                f_index = feature_layer[0]

                tri_loss1 = tri_loss_c1(s_feature[f_index],q_feature[f_index],distance,train_bs) * lambda_t[0]
                for num,f_index in enumerate(feature_layer[1:]):
                    tri_loss1 += tri_loss_c1(s_feature[f_index],q_feature[f_index],distance,train_bs) * lambda_t[num+1]
                tri_loss1 = tri_loss1 / len(feature_layer)
                loss = seg_loss + tri_loss1  
        if len(_query_label)==2:
            f_index = feature_layer[0]
            tri_loss1, tri_loss2 = tri_loss_c2(s_feature[f_index],q_feature[f_index],distance,train_bs)
            tri_loss1, tri_loss2 = tri_loss1 * lambda_t[0], tri_loss2 * lambda_t[0]
            for num,f_index in enumerate(feature_layer[1:]):
                t1, t2 = tri_loss_c2(s_feature[f_index],q_feature[f_index],distance,train_bs)
                tri_loss1 += t1 * lambda_t[num+1]
                tri_loss2 += t2 * lambda_t[num+1]
            tri_loss1, tri_loss2 = tri_loss1 / len(feature_layer), tri_loss2 / len(feature_layer)
            loss = seg_loss + (tri_loss1+tri_loss2)
        if len(_query_label)==3:
            f_index = feature_layer[0]
            tri_loss1, tri_loss2, tri_loss3 = tri_loss_c3(s_feature[f_index],q_feature[f_index],distance,train_bs)
            tri_loss1, tri_loss2, tri_loss3 = tri_loss1 * lambda_t[0], tri_loss2 * lambda_t[0], tri_loss3 * lambda_t[0]
            for num,f_index in enumerate(feature_layer[1:]):
                t1, t2, t3 = tri_loss_c3(s_feature[f_index],q_feature[f_index],distance,train_bs)
                tri_loss1 += t1 * lambda_t[num+1]
                tri_loss2 += t2 * lambda_t[num+1]
                tri_loss3 += t3 * lambda_t[num+1]
            tri_loss1, tri_loss2, tri_loss3 = tri_loss1 / len(feature_layer), tri_loss2 / len(feature_layer), tri_loss3 / len(feature_layer)    
            loss = seg_loss + (tri_loss1+tri_loss2+tri_loss3) 
        ...
        if i_batch % print_freq==0:
            if len(_query_label)==1:
                if SC_DIS == 0:
                    ...
                    print('Epoch {:d} | Episode {:d}/{:d} | Loss {:f}={:f}+{:f}'.format(e, i_batch, len(train_loader), loss,seg_loss,tri_loss1))
            if len(_query_label)==2:
                print('Epoch {:d} | Episode {:d}/{:d} | Loss {:f}={:f}+({:f}+{:f})'.format(e, i_batch, len(train_loader), loss,seg_loss,tri_loss1,tri_loss2))
            if len(_query_label)==3:
                print('Epoch {:d} | Episode {:d}/{:d} | Loss {:f}={:f}+({:f}+{:f}+{:f})'.format(e, i_batch, len(train_loader), loss,seg_loss,tri_loss1,tri_loss2,tri_loss3))
