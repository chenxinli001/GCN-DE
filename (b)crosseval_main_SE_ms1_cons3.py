...
model_path = model_path + ObjectOrgan + '/cons3_mlseg_t0.01_margin500'
    ...
    for k_fold in range(1):
        ...
        for e in range(start_epoch,num_epoch+1):
            ...
            for i_batch, sampled_batch in enumerate(train_loader):
                ...
                seg,sf,qf,qwf = net(condition_input,query_image)
                ...
                if len(_query_label)==1:
                    ...
                    if MEAN == 'DIM':
                        ...
                        qf = torch.mean(qf,dim=1)
                        qwf = torch.mean(qwf,dim=1)
    
                    if len(_query_label)==2:
                        # anchor, pos, neg = qwf_for_seg, sf_for_seg, sf_for_ref
                        qwf_split = torch.split(qwf,bs)
                        sf_split = torch.split(sf,bs)
                        for i in range(2):
                            anchor, pos, neg = qwf_split[i], sf_split[i], sf_split[1-i]
                            #anchor, pos, neg = anchor.view(bs,-1), pos.view(bs,-1), neg.view(bs,-1)
                        ...
                        # anchor, pos  = qf_for_seg, sf_for_seg
                        ...
                        for i in range(3):
                            out_index = [0,1,2]
                            out_index.remove(i)
                            anchor,pos,neg1,neg2 = qwf_split[i], sf_split[i], sf_split[out_index[0]], sf_split[out_index[1]]
                            #anchor, pos, neg1, neg2 = anchor.view(bs,-1), pos.view(bs,-1), neg1.view(bs,-1), neg2.view(bs,-1)
                            dist_p = distance(anchor,pos)
