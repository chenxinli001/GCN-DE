...
model_path = model_path + ObjectOrgan + '/cons1_ss_w_layer9_t1_margin0'
            ...
            for i_batch, sampled_batch in enumerate(train_loader):
                ...
                if len(_query_label)==1:
                    ...
                    if len(_query_label)==2:
                        sc = np.random.choice([0,1])
                        seg, query_label = torch.split(seg,bs)[sc], torch.split(query_label,bs)[sc]
                        seg_loss = criterion1(seg, query_label) + criterion2(seg.squeeze(dim=1), query_label)
        
                        qf_split = torch.split(qf,bs)
                        sf_split = torch.split(sf,bs)
                        anchor, pos, neg = qf_split[sc], sf_split[sc], sf_split[1-sc]
                        ...
                        out_index = [0,1,2]
                        out_index.remove(sc)
                        anchor,pos,neg1,neg2 = qf_split[sc], sf_split[sc], sf_split[out_index[0]], sf_split[out_index[1]]
                        dist_p = distance(anchor,pos)
