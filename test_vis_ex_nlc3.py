...
#model_path = './result_ex/MRI/liver/ex_pretrain_nlc1_[5]/0-fold_best.pth.tar'
model_path = './result_ex2/MRI/liver/base_pretrain_nlc4_[5, 6, 7]/0-fold_best.pth'
k_fold = 0

vis_layer = [5, 6, 7]

# net = fss_feature_ex1(pretrain=True,nlc_layer=[5]).cuda()
net = fss_fea_nlc4(pretrain=True,SE_type=1,nlc_type=4,nlc_layer=[5, 6, 7]).cuda()
...
vis_save_path =  './test_vis_ex_nlc/nlc4'
...
with torch.no_grad():
    ...
    for i_batch, sampled_batch in enumerate(train_loader):
        ...
        # print(len(q_feature),len(qw_feature))
        ...
        path = save_path
            ...
            #     os.makedirs(Fq_path)
            # Fwq_path = path + '/weighted_Q_feature'
            # if not os.path.exists(Fwq_path):
            #     os.makedirs(Fwq_path)
        ...
        attention_map1 = net.segmentor.w5.am.cpu().numpy().squeeze() #s,s
            # qf = q_feature[4].cpu().numpy().squeeze()
            # qwf = qw_feature[4].cpu().numpy().squeeze()
    
            # nl_map = net.segmentor.w6.matrix.cpu().numpy().squeeze() #256,256//4
            # W_y = net.segmentor.w6.W_y.cpu().numpy().squeeze() #C,s,s
            #q_x = net.segmentor.w6.q_x.cpu().numpy().squeeze() #C,s,s
        attention_map2 = net.segmentor.w6.am.cpu().numpy().squeeze() #s,s
            # qf = q_feature[5].cpu().numpy().squeeze()
            # qwf = qw_feature[5].cpu().numpy().squeeze()
        
            # nl_map = net.segmentor.w7.matrix.cpu().numpy().squeeze() #256,256//4
            # W_y = net.segmentor.w7.W_y.cpu().numpy().squeeze() #C,s,s
            #q_x = net.segmentor.w7.q_x.cpu().numpy().squeeze() #C,s,s
        attention_map3 = net.segmentor.w7.am.cpu().numpy().squeeze() #s,s
            # qf = q_feature[6].cpu().numpy().squeeze()
            # qwf = qw_feature[6].cpu().numpy().squeeze()
            ...
            #     cv2.rectangle(img_q, (y0, x0), (y1, x1), (255, 0, 0), 1)
            #     cv2.imwrite(matrix_path+'/S_%d.png' % index, cv2.cvtColor(img_add, cv2.COLOR_BGR2RGB))
            #     cv2.imwrite(matrix_path+'/Q_%d.png' % index, cv2.cvtColor(img_q, cv2.COLOR_BGR2RGB))
            ...
            # attention_map = cv2.resize(attention_map1, dsize=(h, w))
            # attention_map1 = np.uint8(attention_map*255)
            #print(np.unique(attention_map))

            # heat_img = cv2.applyColorMap(attention_map, cv2.COLORMAP_JET)
            # heat_img = cv2.cvtColor(heat_img, cv2.COLOR_BGR2RGB)

        attention_map1 = ndimage.zoom(attention_map1,zoom=(256/attention_map1.shape[0]))
        attention_map2 = ndimage.zoom(attention_map2,zoom=(256/attention_map2.shape[0]))
        attention_map3 = ndimage.zoom(attention_map3,zoom=(256/attention_map3.shape[0]))

        plt.figure()
        plt.subplot(2,3,1)
        plt.imshow(support_image.cpu().numpy().squeeze(),cmap="gray")
        plt.subplot(2,3,2)
        plt.imshow(query_image.cpu().numpy().squeeze(),cmap="gray")
        plt.subplot(2,3,4)
        plt.imshow(attention_map1,cmap="jet")
        plt.subplot(2,3,5)
        plt.imshow(attention_map2,cmap="jet")
        plt.subplot(2,3,6)
        plt.imshow(attention_map3,cmap="jet")
        plt.savefig(path+'/attention.png',format="png")
        plt.show()
        
            # img_q = Q_IMG.copy()
            # img_s = S_IMG.copy()
            
            #cv2.imwrite(path+'/attention.png',heat_img)

            # img_add_s = cv2.addWeighted(img_s, 0.3, heat_img, 0.7, 0)
            # img_add_q = cv2.addWeighted(img_q, 0.3, heat_img, 0.7, 0)

            # cv2.imwrite(path+'/S_add.png' , cv2.cvtColor(img_add_s, cv2.COLOR_BGR2RGB))
            # cv2.imwrite(path+'/Q_add.png' , cv2.cvtColor(img_add_q, cv2.COLOR_BGR2RGB))
