...
def get_args():
    ...
    parser.add_argument("--nlc_layer",type=list,default=[5,6,7])
    parser.add_argument("--SE_type",type=int,default=1)
    parser.add_argument("--nlc_type",type=int,default=4)
...
model_path = model_path + ObjectOrgan + '/base'
...
if nlc_layer:
    model_path += '_nlc%d_%s'%(args.nlc_type,str(nlc_layer))
    ...
    for k_fold in range(1):
        ...
        for e in range(start_epoch,num_epoch+1):
            ...
            for i_batch, sampled_batch in enumerate(train_loader):
                ...
                ac_map = attention.cpu().numpy().squeeze

                if i_batch % 5==0:
                    print('Epoch {:d} | Episode {:d}/{:d} | {:f} ~ {:f}'.format(e, i_batch, len(train_loader), ac_map.min(), ac_map.max()))
