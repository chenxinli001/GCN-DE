...
def get_args():
    ...
    parser.add_argument("--nlc_type",type=int,default=8)
...
if DataSet == 'MRI':
    ...
    encoder_path = './result_pretrain/pretrain_ae/CT/ae_epoch-100.pth'
...
model_path = model_path + ObjectOrgan + '/t{}_cons_'.format(lambda_t)
...
if nlc_layer:
    model_path += 'w_nlc%d_%s_'%(args.nlc_type, str(nlc_layer))
...
if not args.test_vis:
    for k_fold in range(5):
        # net = my_fss_fea().cuda()
        # net = my_fss_fea_nlc2(nlc_layer=nlc_layer).cuda()
        # net = my_fss_fea_nlc2_(nlc_layer=nlc_layer).cuda()
        # net = fss_fea_nlc(args.pretrain,args.SE_type,args.nlc_type,nlc_layer).cuda()
        net = fss_fea_nlc8(args.pretrain,args.SE_type,args.nlc_type,nlc_layer).cuda()
    ...
    with torch.no_grad():
        for k_fold in range(5):
            ...
            dice_list1 = evaluate_fss_kfold_encoder1a(net,whole_image,whole_label,case_start_index,support_item,query_item,save_path2,
                query_label=organ_label_dict[ObjectOrgan]['val'],Num_support=Num_support,test_bs=8,save_img=True)    
            val_dc = sum(dice_list)/len(dice_list)
            val_dc1 = sum(dice_list1)/len(dice_list1)
