...
model_path = model_path + ObjectOrgan + '/ss_cons1_qw2s_layer9_DIM_t1'
...
for k_fold in range(1):
    ...
    best_val_dc, best_val_dc1, best_val_dc2 = -1, - ,-
    ...
    for e in range(start_epoch,num_epoch+1):
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            if len(_query_label)==2:
                ...
                seg, query_label = torch.split(seg,bs)[class_for_seg], torch.split(query_label,bs)[class_for_seg]
                seg_loss = criterion1(seg, query_label) + criterion2(seg.squeeze(dim=1), query_label)

                anchor = torch.split(qf,bs)[class_for_seg]
                pos = torch.split(sf,bs)[class_for_seg]
                neg = torch.split(sf,bs)[1-class_for_seg]
            ...
            if len(_query_label)==3:
                ...
                seg_loss = criterion1(seg, query_label) + criterion2(seg.squeeze(dim=1), query_label)
                out_list = [0,1,2]
                out_list.remove(class_for_seg)
                anchor = torch.split(qf,bs)[class_for_seg]
                pos = torch.split(sf,bs)[class_for_seg]
                neg1, neg2 = torch.split(sf,bs)[out_list[0]], torch.split(s_feature[-1],bs)[out_list[1]]
            ...
            #         print('N=2 dist_p {:.3f}, dist_n {:.3f}, dist_diff {:.3f}, tri_loss {:.3f}'.format(dist_p.mean(),
            #             dist_n.mean(), dist_diff.mean(), tri_loss))
            #     elif len(_query_label)==3:
            #         print('Epoch {:d} | Episode {:d}/{:d} | Loss {:.3f}={:.3f}+{:.3f}x{:.3f}'.format(e, i_batch, len(train_loader), loss,seg_loss,lambda_t,tri_loss))
            #         print('N=3 dist_p {:.3f}, dist_n1 {:.3f}, dist_n2 {:.3f}, dist_diff {:.3f}, tri_loss {:.3f}'.format( \
            #             dist_p.mean(), dist_n1.mean(), dist_n2.mean(), dist_diff.mean(), tri_loss))
