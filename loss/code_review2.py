...
def get_args():
    ...
    parser.add_argument("--dataset", type=str,default='CT',help='MRI,CT')
    ...
    parser.add_argument("--nlc_layer",type=list,default=[2,3])
...
if DataSet == 'MRI':
    ...
    # lambda_t = 50
    lambda_t = args.t
    # model_path = './result_paper4/MRI/'
    model_path = './result/review2/MRI/'
    ...
    # lambda_t = 25
    lambda_t = args.t
    # model_path = './result_paper4/CT/'
    model_path = './result/review2/CT/'
...
if nlc_layer:
    model_path += 'nlc23_%s_'%str(nlc_layer)
...
if not args.test_vis:
    ...
    # for k_fold in range(1):
        # net = my_fss_fea().cuda()
        net = my_fss_fea_nlc23(nlc_layer=nlc_layer).cuda() 
        ...
        for e in range(start_epoch,num_epoch+1):
            ...
            for i_batch, sampled_batch in enumerate(train_loader):
                ...
                if i_batch % print_freq==0:
                    ...
                    f = open(txt_path, "a+")
                    f.write('Epoch {:d} | Episode {:d}/{:d} | Loss {:f} \n'.format(e, i_batch, len(train_loader), loss))
                    f.close()
            ...
            with torch.no_grad():
                ...
                dice_list1 = evaluate_fss_kfold_encoder1a(net,whole_image,whole_label,case_start_index,support_item,query_item,save_path,
                    query_label=organ_label_dict[ObjectOrgan]['val'],Num_support=Num_support,test_bs=10)
