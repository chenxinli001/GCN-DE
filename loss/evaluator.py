...
def evaluate_fss_kfold(net,whole_image,whole_label,case_start_index,support_item,query_item,save_path,query_label,Num_support=6,
    test_bs=10,save_img=False,save_gt=False,epoch=None):
    net.eval()
    #选定support,query
    # support = h5py.File(support_file, 'r')
    # support_volume,support_labelmap = np.asarray(support['Image']),np.asarray(support['Label'])
...
def evaluate_fss_ours1b_kfold(net,whole_image,whole_label,case_start_index,support_item,query_item,
    save_path,query_label,Num_support=6,test_bs=10,Norm='None',save_img=False,save_gt=False):
    ### 精细化support slices的选择
    ### ours1 先尝试最简单的方案，把中心的support_slice和中心的query_slice做匹配，度量标准是逐像素的距离
    ### S和Q分别进行PCA 与位置信息结合，就是说S和Q按区域对应，但具体该Q区域内的每张切片对应S区域内的哪张切片，用PCA计算出来
    ### 欧式距离与位置信息结合
...
def evaluate_pretext_kfold(net,whole_image,whole_label,case_start_index,support_item,query_item,save_path,query_label,Num_support=6,
    test_bs=10,save_img=False,save_gt=False,epoch=None):
    net.eval()
    #选定support,query
    # support = h5py.File(support_file, 'r')
        ...
        for i, query_start_slice in enumerate(query_slice_indexes):
            ...
            for b in range(0, len(query_batch_x), test_bs):
                ...
                try:
                    ...
                    try:
                        out_10,_,_,_ = net(query_batch_x_10)
                        batch_output_10 = out_10>0.5
                    except:
                        out_10,_,_,_,_ = net(query_batch_x_10)
                        batch_output_10 = out_10>0.5
...
def evaluate_fss_kfold_encoder1a(net,whole_image,whole_label,case_start_index,support_item,query_item,
    save_path,query_label,Num_support=6,test_bs=10,Norm='None',save_img=False):
    ### encoder输出的特征空间里度量
...
def eval_func(net,whole_image,whole_label,case_start_index,support_item,query_item,save_path,query_label,Num_support=6,
    ...
    for item in query_item:
        ...
        for i, query_start_slice in enumerate(query_slice_indexes):
            ...
            for b in range(0, len(query_batch_x), test_bs):
                ...
                batch_output_10 = support_batch_x_10[:,1:2]
...
def finetune_pretext_kfold(net,whole_image,whole_label,case_start_index,support_item,query_item,save_path,query_label,Num_support=6,
    test_bs=10,save_img=False,save_gt=False,epoch=None):
    ...
    support_batch_x = np.concatenate([support_volume[:,np.newaxis,:,:],support_labelmap[:,np.newaxis,:,:]],axis=1)[support_slice_indexes]
    support_input = support_batch_x[:,0:1]
    support_label = support_batch_x[:,1]
    support_input_batch = torch.FloatTensor(support_input).cuda()
    support_label_batch = torch.FloatTensor(support_label).cuda()
    max_iter = 10
    for iter in range(max_iter):
        seg = net(support_input_batch)
        loss = criterion1(seg, support_label_batch) + criterion2(seg, support_label_batch)
        if iter==0:
            loss0 = loss
        if iter==max_iter-1:
            loss9 = loss
    ...
    print(f'Finetune num: {len(support_batch_x):d} | Loss: from {loss0.item():.3f} to {loss9.item():.3f}')
