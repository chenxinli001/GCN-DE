...
model_path = \
    './result_paper4/MRI/liver/t0.1_cons_pretrain_nlc22_[5]_run1/0-fold_best.pth.tar'
k_fold = 0

vis_layer = [5, 6]

net = my_fss_fea_nlc22(pretrain=True,nlc_layer=[5, 6],sub_sample=True, bn_layer=True,shortcut=True,
        cos_dis=False).cuda()

checkpoint = torch.load(model_path)
net.load_state_dict(checkpoint['state_dict'])

vis_save_path =  './test_vis3'
...
train_sampler = OneShotBatchSampler(train_dataset.label, 'val', organ_fold_dict[ObjectOrgan],
    batch_size=train_bs, iteration=train_iteration)
    ...
    for i_batch, sampled_batch in enumerate(train_loader):
        ...
        for layer in vis_layer:
            ...
            for index in range(total_region):
                ...
                cv2.imwrite(path+'/%d.png' % index, cv2.cvtColor(img_add, cv2.COLOR_BGR2RGB))
