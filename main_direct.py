...
train_bs = 2 
val_bs = 8
train_iteration = 100#//train_bs 
val_iteration = 100#//val_bs
num_epoch = 20 

reload_mdoel = 0
print_freq = 50

#model_path = './result/direct/'
model_path = './result_pretrain/direct/'
...
_val_dataset = get_class_dataset(val_image_path,val_label_path,1,True)
_val_loader = torch.utils.data.DataLoader(dataset=_val_dataset,batch_size=val_bs,shuffle=False)
...
for e in range(start_epoch,num_epoch+1):
    ...
    for i_batch, sampled_batch in enumerate(train_loader):
        ...
        input = torch.cat([support_image,query_image],dim=0)
        gt = torch.cat([support_label,query_label],dim=0)
        ...
        loss = criterion1(seg, gt) + criterion2(seg, gt)
