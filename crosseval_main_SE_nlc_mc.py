...
def get_args():
    ...
    parser.add_argument("--SE_type",type=int,default=1)
    parser.add_argument("--nlc_type",type=int,default=9)

    parser.add_argument("--mc_t",type=float,default=0)
...
if DataSet == 'MRI':
    ...
    num_epoch = 15
    Num_support = 8 
    model_path = './result_correct2/MRI/'
    ...
    num_epoch = 25
    Num_support = 8
    model_path = './result_correct2/CT/'

model_path = model_path + ObjectOrgan + '/base_mc{}_'.format(args.mc_t)
...
if nlc_layer:
    model_path += 'nlc%d_%s_'%(args.nlc_type,str(nlc_layer))
...
if not args.test_vis:
    for k_fold in range(5):
        #net = fss_fea_nlc(args.pretrain,args.SE_type,args.nlc_type,nlc_layer).cuda()
        #net = fss_fea_nlc4(args.pretrain,args.SE_type,args.nlc_type,nlc_layer).cuda()
        #net = fss_fea_nlc1(args.pretrain,args.SE_type,args.nlc_type,nlc_layer).cuda()
        net = fss_fea_nlc9(args.pretrain,args.SE_type,args.nlc_type,nlc_layer).cuda()
        ...
        for e in range(start_epoch,num_epoch+1):
            ...
            for i_batch, sampled_batch in enumerate(train_loader):
                ...
                seg,s_feature,q_feature,qw_feature,matrix = net(condition_input,query_image)

                #print(matrix.shape)
                query_mask = F.interpolate(query_label.unsqueeze(1), size=[16, 16])
                support_mask = F.interpolate(support_label.unsqueeze(1), size=[16, 16])
                bs,_,h,w = query_mask.size()
                query_mask = query_mask.view(bs,h*w,1)
                support_mask = support_mask.view(bs,1,h*w)
                qs_matrix = torch.matmul(query_mask,support_mask) #bs,hw,hw
                #print(qs_matrix.min(),qs_matrix.max())

                mc_loss = l2_loss(matrix,qs_matrix)
                #print(mc_loss)
            
                seg_loss = criterion1(seg, query_label) + criterion2(seg, query_label)
                loss = seg_loss + mc_loss * args.mc_t
                ...
                if i_batch % print_freq==0:
                    print('Epoch {:d} | Episode {:d}/{:d} | Loss {:f} = {:f} + {:f}* {:f}'.format(e, \
                    i_batch, len(train_loader), loss,seg_loss,args.mc_t,mc_loss))
                    # print('Epoch {:d} | Episode {:d}/{:d} | Loss {:f} = {:f} '.format(e, \
                    # i_batch, len(train_loader), loss,seg_loss))
